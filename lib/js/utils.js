$(window).on('load', function () {
    ...
    highlightTab(); // highlight nav tab
})
...
function highlightTab() {
    let tabLink = localStorage.getItem('tab-link') || 'index';
    // $('.header__nav > ul > li > a').each(function () {
    //     if ($(this).attr('href').includes(tabLink)) {
    //         $(this).addClass('active-link');
    //     }
    // })
    const currentUrl = window.location.href;
    const htmlFileName = currentUrl.substring(currentUrl.lastIndexOf('/') + 1, currentUrl.lastIndexOf('.'));
    if (tabLink.includes(htmlFileName)) {
        $('.header__nav > ul > li > a').each(function () {
            if ($(this).attr('href').includes(tabLink)) {
                $(this).addClass('active-link');
            }
    ...
    $('.header__nav > ul > li > a').on('click', function () {
        localStorage.setItem('tab-link', $(this).attr('href'));
    })
}